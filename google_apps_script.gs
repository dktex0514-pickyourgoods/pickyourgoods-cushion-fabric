...
// ⚙️  업로드 이미지 최대 크기 (base64 디코딩 후 바이트)
const MAX_IMAGE_BYTES = 10 * 1024 * 1024; // 10MB


// ────────────────────────────────────────────────────────────────
//  🔒 보안 유틸: HTML 이스케이프 + 입력 정화 + 레이트 리밋
// ────────────────────────────────────────────────────────────────
function escHtml(s) {
...
function isValidEmail(s) {
  ...
  return /^[^\s@<>"]+@[^\s@<>"]+\.[a-zA-Z]{2,24}$/.test(t);
}

function sanitizeName(s) {
  return stripNewlines(s).replace(/[<>"']/g, '').substring(0, 50);
}

function sanitizePhone(s) {
  return String(s || '').replace(/[^0-9+\- ]/g, '').substring(0, 20);
}

// 레이트 리밋 — CacheService 사용, 액션별 버킷 + 디바이스 핑거프린트
function checkRateLimit(bucket, maxPerWindow, windowSec, fingerprint) {
  try {
    const cache = CacheService.getScriptCache();
    const key = 'rl_' + bucket + '_' + (fingerprint || 'global');
    const cur = Number(cache.get(key) || 0);
    if (cur >= maxPerWindow) {
      return { ok: false, retry: windowSec };
    }
    cache.put(key, String(cur + 1), windowSec);
    return { ok: true };
  } catch (e) {
    return { ok: true }; // 캐시 실패 시 차단하지 않음 (가용성 우선)
  }
}
...
function doPost(e) {
  try {
    ...
    // ── 레이트 리밋: 액션별 제한 (토큰은 공개되어 있으므로 추가 보호 필수) ──
    const fp = (data.email || '') + '|' + String(data.phone || '').substring(0, 4);
    const action = data.action || 'order';
    const limits = {
      'gemini_transform': { max: 10, win: 600 },   // 10분당 10건
      'remove_bg':        { max: 15, win: 600 },   // 10분당 15건
      'order':            { max: 5,  win: 600 }    // 10분당 5건
    };
    const lim = limits[action] || limits.order;
    const rl = checkRateLimit(action, lim.max, lim.win, fp);
    if (!rl.ok) {
      return jsonResponse({ success: false, error: '요청이 너무 많습니다. 잠시 후 다시 시도해주세요.' });
    }

    // ── 이미지 크기 검증 (모든 action 공통) ──
    if (data.imageData && typeof data.imageData === 'string') {
      const approxBytes = Math.floor(data.imageData.length * 0.75);
      if (approxBytes > MAX_IMAGE_BYTES) {
        return jsonResponse({ success: false, error: '이미지 크기가 너무 큽니다 (10MB 이하만 허용).' });
      }
    }
...
function handleOrder(data) {
  // 서버사이드 입력 정화 (이메일 인젝션 / XSS 방지)
  const name      = sanitizeName(data.name)  || '(이름 없음)';
  const phone     = sanitizePhone(data.phone) || '(연락처 없음)';
  const rawEmail  = stripNewlines(data.email);
  const email     = rawEmail && isValidEmail(rawEmail) ? rawEmail : '';
  const product   = stripNewlines(data.product)  || '(상품 미지정)';
  const material  = stripNewlines(data.material) || '';
  const size      = stripNewlines(data.size)     || '(사이즈 미지정)';
  ...
  // 3) 고객에게 접수 확인 이메일 (이메일 형식이 올바를 때만)
  ...
  // 사장님 메일 실패 시 success=false (클라이언트가 실패 인지 가능하도록)
  return jsonResponse({
    success: ownerMailOk,
...
function sendOwnerEmail(name, phone, email, product, material, size, price, fileUrl, timestamp) {
  // 제목에서 CR/LF 제거 (헤더 인젝션 방지), HTML은 escHtml로 이스케이프
  const subject = stripNewlines('[픽유어굿즈 쿠션/패브릭] 새 주문 — ' + name + ' / ' + phone);

  const materialRow = material
    ? '<tr><td style="padding:8px 12px;background:#f5f5f5;font-weight:700;">소재</td>'
    + '<td style="padding:8px 12px;border-bottom:1px solid #eee;">' + escHtml(material) + '</td></tr>'
    : '';
  ...
  const htmlBody = '<div style="font-family:\'Noto Sans KR\',sans-serif; max-width:520px;">'
    ...
    + '<td style="padding:8px 12px;border-bottom:1px solid #eee;">' + escHtml(email) + '</td></tr>'
    + '<tr><td style="padding:8px 12px;background:#fff5f0;font-weight:700;">상품</td>'
    + '<td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:700;color:#d2691e;">' + escHtml(product) + '</td></tr>'
    + materialRow
    + '<tr><td style="padding:8px 12px;background:#fff5f0;font-weight:700;">사이즈</td>'
    + '<td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:700;color:#d2691e;">' + escHtml(size) + '</td></tr>'
    + '<tr><td style="padding:8px 12px;background:#fff5f0;font-weight:700;">금액</td>'
    + '<td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:700;color:#d2691e;">' + escHtml(Number(price).toLocaleString('ko-KR')) + '원</td></tr>'
    + '<tr><td style="padding:8px 12px;background:#f5f5f5;font-weight:700;">디자인 파일</td>'
    + '<td style="padding:8px 12px;"><a href="' + escHtml(fileUrl) + '" style="color:#0a6cf5;font-weight:700;">여기를 클릭하여 확인</a></td></tr>'
    + '</table>'
    + '<p style="margin-top:20px;font-size:13px;color:#888;">접수 시각: ' + escHtml(timestamp) + '</p>'
    + '</div>';
...
function sendCustomerEmail(name, phone, email, product, material, size, price, fileUrl, file, timestamp) {
  const subject = stripNewlines('[픽유어굿즈] 주문 접수 확인 — ' + name + '님');

  const materialInfo = material ? '<p>소재: <strong>' + escHtml(material) + '</strong></p>' : '';
  ...
  const htmlBody = '<div style="font-family:\'Noto Sans KR\',sans-serif; max-width:560px; color:#222;">'
    ...
    + '<p style="margin-top:16px;">안녕하세요, <strong>' + escHtml(name) + '</strong>님!</p>'
    ...
    + '<p>이름: ' + escHtml(name) + '</p>'
    + '<p>연락처: ' + escHtml(phone) + '</p>'
    + '<p>상품: <strong>' + escHtml(product) + '</strong></p>'
    + materialInfo
    + '<p>사이즈: <strong>' + escHtml(size) + '</strong></p>'
    + '<p>금액: <strong>' + escHtml(Number(price).toLocaleString('ko-KR')) + '원</strong></p>'
    + '</div>'
    + '<p style="margin-top:24px;font-size:12px;color:#aaa;">문의사항이 있으시면 ' + escHtml(OWNER_EMAIL) + ' 로 연락주세요.</p>'
    + '</div>';
